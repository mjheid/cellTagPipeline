...
function usage {
  ...
  echo "Arguments:"
  echo "  --mkref    Run cellranger mkref. Default: false"
  echo "  --count    Run cellranger count. Default: false"
  echo "  --filter_sam    Run samtools to filter .bam provided by cellranger count."
  echo "                  Default: false"
  echo "  --genome [Path]   Path from current dir to fasta file of genome. Required "
  echo "                    for --mkref. Default /sourceFile/genome_new.fa"
  echo "  --genes [Path]   Path from current dir to .gtf file of genes of genome. "
  echo "                   Required for --mkref. Default: /sourceFile/genes_new.gtf"
  echo "  --refgenome [String]   Name of reference genome. Needed for --mkref as output "
  echo "                         dir name and --count as input. Default: refdata-gex-GRCh38-2020-A-cellTag-barcode"
  echo "  --sample_name [String]   Name of Sample, output from ilumina. Used for --count"
  echo "                           as well as --filter_sam. Default: SI-TT-H4"
  echo "  --fastqs [Path]   Path from current dir to fastqs of ilumina run. Contains "
  echo "                    --sample_name samples. Used for --count. Default: None"
  echo "  --expect_cells [int]   Number of expected cells for --count. Default: 5000"
  echo "  --GFP [Path]   Path from current dir to GFP.CDS file, for filtering for "
  echo "                 reads mapped to this sequence. Used with --filter_sam. Default: data/GFP.CDS"
  echo "  --UTR [Path]   Path from current dir to CellTag.UTR file, for filtering for"
  echo "  --GFP [Path]   reads mapped to this sequence. Used with --filter_sam. Default data/CellTag.UTR"
  echo "  --bam_data [Path]   Path from current dir to output dir of filtered .bam "
  echo "                      files when using --filter_sam. Default: data/bam"
