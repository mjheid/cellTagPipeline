...
function usage {
  ...
  echo "Arguments:"
  echo "  --visualize    Visualize filtering steps. Default: false"
  echo "  --out [Path]   Path from current dir to dir where produced files"
  echo "                 should get saved. Default: data/out/"
  echo "  --collapsing_name [String]   Name of collapsing file, used for "
  echo "                               starcode. Default: collapsing.txt"
  echo "  --save_progress_name [String]   Name of outputs. Default: test"
  echo "  --sample_name [String]   Name of Sample, output from ilumina. "
  echo "                           Used to find path barcodes.tsv. Default: SI-TT-H4"
  echo "  --whitelist_version [String]   Version of the whitelist. Either v1, v2 or v3."
  echo "                                 Default: v1"
  echo "  --whitelist_path [Path]   Path from current dir to the whitelist csv, gets adujusted."
  echo "                            based on specified version. Default: data/whitelist/V1.CellTag.Whitelist.csv"
  echo "  --high_filter [int]   Upper bound for filtering celltags. Default: 20"
  echo "  --low_filter [int]   Lower bound for filtering celltags. Default: 2"
  echo "  --bam_data [Path]   Path from current dir to output dir of filtered .bam "
  echo "                      files when using --filter_sam. Default: data/bam/possorted_genome_bam.filtered.bam"
  echo "  --tagged [int]   When binarizing, the number of cellTags needed to"
  echo "                   consider cell as tagged. Default: 2"
  echo "  --jac_cut [float]   When calculating simiarity of cells voa Jaccard coefficiants, "
  echo "                      this is the cut-off used to consider cells for clone calling. Default: 0.7"
